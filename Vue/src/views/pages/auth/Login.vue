...
import {responseToastConfig, addClassById, globalConfig} from '../../../service/globalQuote';
...
const switchApi = () => {
    ...
    console.log(apiUse.value);
};

// Api可用性监听
watch(apiAvailability, (newValue) => {
    if (newValue === 0) {
        console.log('API 可用性已降至 0');
        // 中断一言更新函数
        updateYiyan = null;
        // 清除一言定时器
        clearTimer();
                ...
                <div class="w-full surface-card pt-7 pb-8 px-5 sm:px-8" style="border-radius: 53px">
                    <div class="text-center mb-5">
                        ...
                        <div style="width: 330px; height: 20px">
                            <span class="transition: opacity 0.5stext-600 font-bold ayou" style="transition: opacity 0.5s">{{ yiyan }}</span>
                        </div>
                    </div>

                    <div class="pt-3">
                        <span class="p-float-label mb-5">
                            <InputText id="accountLoginId" v-model="accountLoginId" class="w-full p-3" @blur="accountLoginIdIsFocus = false" @focus="accountLoginIdIsFocus = true" />
                            <label :class="{ 'text-600': accountLoginIdIsFocus || accountLoginId.length > 0, 'text-500': !accountLoginIdIsFocus }" class="font-bold ml-1" for="username">Account ID</label>
                        </span>
                        ...
                        <span class="p-float-label mb-4">
                            ...
                            ></Password>
                            <label :class="{ 'text-600': passwordIsFocus || accountLoginPassword.length > 0, 'text-500': !passwordIsFocus }" class="font-bold ml-1" for="password">Password</label>
                        </span>
                        ...
                        <div class="flex align-items-center justify-content-between pt-3 mt-0 gap-5">
                            ...
                            <a class="font-medium no-underline ml-2 text-right cursor-pointer" style="color: var(--primary-color)" @click="router.push({ path: '/auth/register' })">注册账号</a>
                        </div>
                    </div>
                </div>
            </div>
